...
var paper = new function (undefined) {
        ...
        function inject(dest, src, enumerable, base, preserve) {
          ...
          function field(name, val, dontCheck) {
            ...
            var isFunc = typeof val === 'function',
              ...
              bean;
            if ((dontCheck || val !== undefined && src.hasOwnProperty(name)) && (!preserve || !prev)) {
              if (isFunc && prev)
                val.base = prev;
              if (isFunc && beans && (bean = name.match(/^([gs]et|is)(([A-Z])(.*))$/)))
    ...
    var Matrix = Base.extend({
      ...
      toString: function () {
        ...
        return '[[' + [f.number(this._a), f.number(this._b),
          f.number(this._tx)
        ].join(', ') + '], [' + [f.number(this._c), f.number(this._d),
          f.number(this._ty)
        ].join(', ') + ']]';
      },
      ...
      draw: function (ctx, matrix, pixelRatio) {
        ...
        if (this._selectedItemCount > 0) {
          ...
          for (var id in this._selectedItems) {
            ...
            half = size / 2;
            if (item._updateVersion === this._updateVersion && (item._drawSelected || item._boundsSelected)) {
    ...
    var Item = Base.extend(Callback, {
      ...
      function (name) {
        ...
        this['set' + part] = function (value) {
          if (value != this[name]) {
            this[name] = value;
            this._changed(name === '_locked' ? 32 : 33);
          }
        };
      }, {}), {
      ...
      hitTest: function (point, options) {
        ...
        if (!this._children && !this.getInternalRoughBounds()
          ...
          return null;
        var checkSelf = !(options.guides && !this._guide || options.selected && !this._selected || options.type && this._type !== options.type),
          that = this,
          res;
      ...
      isGroupedWith: function (item) {
        ...
        while (parent) {
          if (parent._parent && /^(group|layer|compound-path)$/.test(parent._type) && item.isDescendant(parent))
            return true;
      ...
      draw: function (ctx, param) {
        ...
        var blendMode = this._blendMode,
          ...
          direct = normalBlend && opacity === 1 || (nativeBlend || normalBlend && opacity < 1) && this._canComposite(),
    ...
    var Shape = Item.extend({
        ...
        _equals: function (item) {
          return this._shape === item._shape && this._size.equals(item._size) && Base.equals(this._radius, item._radius);
        },
        ...
        getRadius: function () {
          ...
          return this._shape === 'circle' ? rad : new LinkedSize(rad.width, rad.height, this, 'setRadius');
        },
      ...
      _hitTest: function (point, options) {
        ...
        function checkSegmentStroke(segment) {
          if (join !== 'round' || cap !== 'round') {
            ...
            if (closed || segment._index > 0 && segment._index < segments.length - 1) {
        ...
        if (options.ends && !options.segments && !closed) {
          if (res = checkSegmentPoints(segments[0], true) || checkSegmentPoints(segments[segments.length - 1], true))
            return res;
    ...
    var Style = Base.extend(new function () {
      ...
      Base.each(defaults, function (value, key) {
        ...
        fields[set] = function (value) {
          ...
          if (children && children.length > 0 && this._item._type !== 'compound-path') {
            for (var i = 0, l = children.length; i < l; i++)
        ...
        fields[get] = function (_dontMerge) {
          ...
          if (!children || children.length === 0 || _dontMerge || this._item instanceof CompoundPath) {
      ...
      getFontStyle: function () {
        ...
        return this.getFontWeight() + ' ' + size + (/[a-z]/i.test(size + '') ? ' ' : 'px ') + this.getFontFamily();
      },
    ...
    Base.extend(Callback, {
      ...
      initialize: function Palette(title, components, values) {
        var parent = DomElement.find('.palettejs-panel') || DomElement.find('body').appendChild(
          DomElement.create('div', {
            'class': 'palettejs-panel'
          }));
        this._element = parent.appendChild(
          DomElement.create('table', {
    ...
    var Component = Base.extend(Callback, {
      ...
      initialize: function Component(obj) {
        ...
        this._type = obj.type in this._types ? obj.type : 'options' in obj ? 'list' : 'onClick' in obj ? 'button' : typeof obj.value;
        ...
        this._element = DomElement.create('tr', [
          'td', [this._label = DomElement.create('label', {
            'for': id
          })],
          'td', [this._input]
        ]);
      ...
      function compile(code) {
        ...
        function walkAST(node, parent) {
          ...
          case 'AssignmentExpression':
            if (!(parent && (
              parent.type === 'ForStatement' || parent.type === 'BinaryExpression' && /^[=!<>]/.test(parent.operator) || parent.type === 'MemberExpression' && parent.computed))) {
        ...
        walkAST(scope.acorn.parse(code, {
          ranges: true
        }));
        return code;
      }
